...
EntryPointDiscoveryStrategy::EntryPointDiscoveryStrategy(cpu::CpuPlugin* cpu,
...
void EntryPointDiscoveryStrategy::RecordLeaTarget(uint32_t address) {
  lea_targets_.insert(address);
}

bool EntryPointDiscoveryStrategy::IsLeaTarget(uint32_t address) const {
  return lea_targets_.count(address) > 0;
}

const std::set<uint32_t>& EntryPointDiscoveryStrategy::GetLeaTargets() const {
  return lea_targets_;
}
...
void EntryPointDiscoveryStrategy::ScanForSubroutinePatterns(
  ...
  for (uint32_t addr = start; addr < end; addr += kSampleStride) {
...
void EntryPointDiscoveryStrategy::DiscoverEntryPoints(
  ...
  ScanForSubroutinePatterns(address_map, discovered_entry_points);
