...
GraphicsDetectionStrategy::GraphicsDetectionStrategy(const core::Binary* binary)
    : binary_(binary) {}

bool GraphicsDetectionStrategy::IsValidAddress(uint32_t address) const {
  uint32_t start = binary_->load_address();
  uint32_t end = start + binary_->size();
  return address >= start && address < end;
}

float GraphicsDetectionStrategy::CalculateEntropy(const uint8_t* data,
                                                  size_t length) const {
...
bool GraphicsDetectionStrategy::HasBitmapEntropy(uint32_t start,
                                                 uint32_t end) const {
...
bool GraphicsDetectionStrategy::HasByteAlignment(uint32_t start,
                                                 uint32_t end) const {
  uint32_t region_size = end - start + 1;
...
bool GraphicsDetectionStrategy::IsInGraphicsRegion(uint32_t start,
                                                   uint32_t end) const {
...
bool GraphicsDetectionStrategy::HasSpritePatterns(uint32_t start,
                                                  uint32_t end) const {
  uint32_t region_size = end - start + 1;
